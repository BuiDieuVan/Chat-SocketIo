...
const PrivateRoom = require('./room/private.js');

const auth = require('./user/auth').auth;
const User = require('./user/user.js').User;
const Token = require('./user/user.js').Token;
...
const main = io.on('connection', (socket) => {
    ...
    socket.on('send clientId', (id) => {
        ...
        if (!connectClient) {
            return io.to(socket.id).emit('user connect error');
        }
        // if (!connectClient) 
